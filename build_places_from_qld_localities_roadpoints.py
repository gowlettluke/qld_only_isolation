...
For each official QLD locality polygon:
  ...
  2. choose the road node closest to the locality's representative centre;
  3. use that road-node coordinate as the place point;
...
def load_graph_nodes(graph_path: Path) -> Tuple[List[Any], List[Point], List[Tuple[float, float]], STRtree]:
    ...
    tree = STRtree(points)
    return node_ids, points, xy_m, tree
...
def choose_road_node_inside_polygon(
    ...
    xy_m: List[Tuple[float, float]],
    tree: STRtree,
) -> Tuple[Optional[Any], Optional[float], Optional[float], Optional[float]]:
    ...
    p = points[best_idx]
    return node_ids[best_idx], float(p.y), float(p.x), math.sqrt(best_d2)
...
def feature_to_place(
    ...
    node, road_lat, road_lon, offset_m = choose_road_node_inside_polygon(
        geom,
        centre_lon,
        centre_lat,
        node_ids,
    ...
    if node is not None:
        lat = road_lat
        lon = road_lon
        point_method = "road_node_inside_locality"
        graph_node = str(node)
...
def build_places(graph_path: Path, existing_path: Optional[Path], output_path: Path, page_size: int) -> None:
    ...
    node_ids, points, xy_m, tree = load_graph_nodes(graph_path)
    ...
    for i, f in enumerate(features, start=1):
        row = feature_to_place(f, node_ids, points, xy_m, tree, hub_by_name)
        if row is None:
            skipped += 1
            continue
        rows.append(row)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build road-aware places.csv from official Queensland locality polygons.",
    ...
    parser.add_argument("--output", default="places_all_qld_localities_roadpoints.csv", help="Output CSV path.")
